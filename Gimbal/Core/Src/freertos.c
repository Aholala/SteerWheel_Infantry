...
void MX_FREERTOS_Init(void) {
  ...
  if (robot_init(robot_get()) != BSP_STATUS_OK)
  {
    Error_Handler();
  }
  ...
  if (robot_get()->control.chassis.initialized)
  {
  ...
  if (robot_get()->control.gimbal.initialized)
  {
    GimbalTaskHandle = osThreadNew(StartGimbalTask, NULL, &GimbalTask_attributes);
  }

  /* creation of IMUTask */
  if (robot_get()->control.imu.initialized)
  {
  ...
  if (robot_get()->control.shooter.initialized)
  {
