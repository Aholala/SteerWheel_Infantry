...
void board_config_set_bmi088_chip_select(bool gyroscope, bool is_selected)
{
    GPIO_TypeDef *const port = gyroscope ? BMI088_GYRO_CS_GPIO_Port : BMI088_ACCEL_CS_GPIO_Port;
    const uint16_t pin = gyroscope ? BMI088_GYRO_CS_Pin : BMI088_ACCEL_CS_Pin;
    HAL_GPIO_WritePin(port, pin, is_selected ? GPIO_PIN_RESET : GPIO_PIN_SET);
}

void board_config_delay_ms(uint32_t delay_ms)
{
    HAL_Delay(delay_ms);
}

uint32_t board_config_get_time_us(void)
{
    return DWT->CYCCNT / (BOARD_CONFIG_CPU_FREQUENCY_HZ / 1000000UL);
}
