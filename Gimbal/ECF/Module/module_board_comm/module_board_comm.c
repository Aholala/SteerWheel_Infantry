                                                             ...
                                                             uint8_t flags, uint8_t sequence)
{
    bsp_can_frame_t frame = {
        .identifier = me->transmit_base_identifier + (uint32_t)message,
                                                  ...
                                                  const module_board_comm_config_t *config)
{
    ...
    if ((me == NULL) || (config == NULL) || (config->can == NULL) ||
        ...
        (config->receive_base_identifier >
         (0x7FFU - ((uint32_t)MODULE_BOARD_COMM_MESSAGE_COUNT - 1U))) ||
        (config->transmit_base_identifier == config->receive_base_identifier))
    {
        return MODULE_BOARD_COMM_STATUS_INVALID_ARGUMENT;
    }
    (void)memset(me, 0, sizeof(*me)); // 清零对象
    me->can = config->can;
    me->transmit_base_identifier = config->transmit_base_identifier;
    me->receive_base_identifier = config->receive_base_identifier;
                                                          ...
                                                          const bsp_can_frame_t *frame)
{
    ...
    if ((frame->id_type != BSP_CAN_ID_STANDARD) || (frame->frame_type != BSP_CAN_FRAME_DATA) ||
        ...
        (frame->identifier < me->receive_base_identifier) ||
        (frame->identifier >=
         (me->receive_base_identifier + (uint32_t)MODULE_BOARD_COMM_MESSAGE_COUNT)))
    {
        return MODULE_BOARD_COMM_STATUS_INVALID_FRAME;
    }

    message =
        (module_board_comm_message_t)(frame->identifier - me->receive_base_identifier);
