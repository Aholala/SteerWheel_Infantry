                                              ...
                                              float *output)
{
    ...
    // 卡尔曼增益 K = P / (P + R)
    prior_covariance = me->covariance;
    me->gain = prior_covariance / innovation_covariance;

    // 状态更新 x = x + K*(z - x)
    me->estimate += me->gain * (measurement - me->estimate);

    // 协方差更新 P = (1 - K) * P
    residual_gain = 1.0F - me->gain;
    me->covariance = residual_gain * residual_gain * prior_covariance +
                     me->gain * me->gain * me->measurement_noise;
