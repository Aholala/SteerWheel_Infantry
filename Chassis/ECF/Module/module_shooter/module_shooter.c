...
static module_shooter_status_t module_shooter_set_motor_targets(module_shooter_t *me)
{
    ...
    if (me->has_local_friction &&
        ((module_motor_set_target(me->left_friction_motor, left_velocity_rad_per_s) !=
                                            ...
                                            const module_shooter_config_t *config)
{
    ...
    if ((me == NULL) || (config == NULL) ||
        ((config->left_friction_motor == NULL) != (config->right_friction_motor == NULL)) ||
        ((config->left_friction_motor == NULL) && (config->feeder_motor == NULL)) ||
        ((config->left_friction_motor != NULL) &&
         (!config->left_friction_motor->is_initialized ||
          !config->right_friction_motor->is_initialized)) ||
        ((config->feeder_motor != NULL) && !config->feeder_motor->is_initialized) ||
    ...
    *me = (module_shooter_t){
        ...
        .has_local_friction = config->left_friction_motor != NULL,
        .has_local_feeder = config->feeder_motor != NULL,
...
module_shooter_status_t module_shooter_enable(module_shooter_t *me)
{
    ...
    // 使能三个电机
    if ((me->has_local_friction &&
         ((module_motor_enable(me->left_friction_motor) != MODULE_MOTOR_STATUS_OK) ||
    ...
    // 读取拨弹电机当前位置，作为初始目标
    feeder_feedback = me->has_local_feeder ? module_motor_get_feedback(me->feeder_motor) : NULL;
    if (me->has_local_feeder && ((feeder_feedback == NULL) || !feeder_feedback->is_online))
    {
...
module_shooter_status_t module_shooter_disable(module_shooter_t *me)
{
    ...
    // 禁用三个电机
    if ((me->has_local_friction &&
         ((module_motor_disable(me->left_friction_motor) != MODULE_MOTOR_STATUS_OK) ||
...
module_shooter_status_t module_shooter_update(module_shooter_t *me, float delta_time_s)
{
    ...
    // ---- 计算位置误差（目标 - 实际） ----
    position_error_rad = me->has_local_feeder
                             ? me->feeder_target_position_rad - feeder_feedback->position_rad
                             : 0.0F;
...
module_shooter_status_t module_shooter_set_external_friction_ready(module_shooter_t *me,
                                                                   bool is_ready)
{
    if (me == NULL)
    {
